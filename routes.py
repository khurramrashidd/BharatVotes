...
def voter_dashboard():
    ...
    # --- FIX: Convert UTC to IST (Add 5 Hours 30 Minutes) ---
    if vote_record:
        # Create a display timestamp so we don't mess up the database
        # IST = UTC + 5:30
        vote_record.timestamp = vote_record.timestamp + timedelta(hours=5, minutes=30)
    # --------------------------------------------------------

    # 4. Pass both 'voter' and 'vote_record' to the template
